...
if option == 'lenet5':
    ...
    prototxt = '3_prototxt_solver/L2/train_val_codebook.prototxt'             
...
def quantize_update_net(net, codebook):
    ...
    for layer in layers:
        ...
        codebook_size = len(codebook[layer])
        shifts = np.array(range(W.size)) * codebook_size + codes
        mask_code = np.zeros((codebook_size * W.size))
        mask_code[shifts] = 1
        mask_code = np.reshape(mask_code, (codebook_size, W.size))

        mask_rowsum = np.sum(mask_code, axis = 1)

        np.copyto(net.params[layer][0].data, W_q)
        print "data copied"
        sys.stdout.flush()
        np.copyto(net.params[layer][0].maskcode, mask_code)
        print "maskcode copied"
        sys.stdout.flush()
        np.copyto(net.params[layer][0].maskcode_rowsum, mask_rowsum)
        print "maskcode rowsum copied"
        sys.stdout.flush()
        net.parmas[layer][0].codebooksize = codebook_size
        print "codebooksize copied"
        sys.stdout.flush()
