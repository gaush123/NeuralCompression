...
def kmeans_net(net, layers, num_c = 16, initials=None):
    ...
    for idx, layer in enumerate(layers):
        ...
        if initials is None: #Default: uniform sample
            ...
            codebook[layer],dist = scv.kmeans(W, initial_uni)
        elif type(initials) == type(np.array([])):
            codebook[layer],dist = scv.kmeans(W, initials)
        else:
            print type(initials)
    return codebook
...
def quantize_net(net, codebook):
    ...
    for layer in layers:
        ...
        codebook_m = np.append(codebook[layer], 0.0)
        codes, dist = scv.vq(W.flatten(), codebook_m)
        W_q = np.reshape(codebook_m[codes], W.shape)
